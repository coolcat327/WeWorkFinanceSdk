...
int main(int argc, char *argv[])
{
    ...
    // 添加从环境变量获取企业微信配置的corpid与secrectkey
    const char* corpid = getenv("CORPID");
    const char* secrectkey = getenv("SECRECTKEY");
    // 检查是否成功获取到环境变量的值
    if (corpid == NULL || secrectkey == NULL) {
        fprintf(stderr, "Failed to get environment variables.\n");
        return 1;
    }
    ret = init_fn(sdk, corpid, secrectkey);
    if (ret != 0)
    {
        // sdk需要主动释放
        destroysdk_fn(sdk);
